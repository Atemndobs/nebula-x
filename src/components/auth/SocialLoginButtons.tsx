...
const SocialButton = ({
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`w-full flex items-center justify-center px-4 py-2 rounded-md shadow-sm text-sm font-medium transition-colors duration-200 ${baseStyles[provider]}`}
      aria-label={`Sign in with ${label}`}
    >
      <span className={`mr-2 ${isLoading ? 'opacity-0' : 'opacity-100'} transition-opacity`}>
        {icon}
      </span>
      <span className="flex-1 text-center">
        {isLoading ? 'Signing in...' : `Continue with ${label}`}
      </span>
      {/* Invisible icon to maintain layout when loading */}
